...
class MY_Parser extends CI_Loader {
    ...
    * @param mixed $caching
    */
    public function parse($template, $data = array(), $return = FALSE, $caching = TRUE, $cache_id = NULL)
    {
        ...
        if ($caching === FALSE)
        {
            $this->CI->smartyview->disable_caching();
        }
        
        // If no file extension dot has been found default to defined extension for view extensions
        if ( !stripos($template, '.') ) 
        {
            $template = $template.".".$this->CI->smartyview->osmarty()->template_ext;
        }
        ...
        if (!empty($data))
        {
            ...
            {
                $this->CI->smartyview->osmarty()->assign($key, $val);
            }
        }
        
        // Get our template data as a string
        $template_string = $this->CI->smartyview->osmarty()->fetch($template,$cache_id);
    ...
    */
    ...
    {
        return $this->CI->smartyview->osmarty()->fetch('string:'.$template, $data);
    }
